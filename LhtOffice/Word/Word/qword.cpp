...
void QWord::insertPic(const QString &picPath)
{
    QAxObject *  bookmark_pic  = m_word->querySubObject("Selection");

    if(!bookmark_pic->isNull())
    {

        QAxObject* range = bookmark_pic->querySubObject("Range");
        range->querySubObject("InlineShapes")->dynamicCall("AddPicture(const QString&)",picPath); //插入图片
